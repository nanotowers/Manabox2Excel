...
REQUEST_DELAY = 0.15   # Scryfall pide 50-100ms; damos margen
...
# Agrupa la misma impresión repartida en varias carpetas en una sola tarjeta,
# sumando las copias. Ponlo en False para ver cada entrada del CSV por separado.
AGRUPAR_IMPRESIONES = True

# Las cartas cuyo binder de Manabox empieza por este prefijo se consideran
# MAZOS ARMADOS, no material de intercambio. Se excluyen de la portada y pasan
# a su propia sección. Ejemplo de binder: "EDH Gishath", "EDH Sephiroth".
PREFIJO_MAZO = "EDH"

# ── Reglas por binder ─────────────────────────────────────────────────────────
# Todas comparan por PREFIJO y sin distinguir mayúsculas, así que "JS" cubre
# "JS Avatar" y "JS 25 Foundations".

# Vitrina: piezas de colección. Salen del catálogo de intercambio pero siguen
# disponibles en el constructor de mazos.
BINDERS_EXHIBICION = ["Secret Lair"]

# Ocultos: no aparecen en ninguna parte de la web. Siguen en tu Excel.
BINDERS_OCULTOS = [
    "JS",                # jumpstart sin desprecintar
    "No encontrada",     # se registraron pero no aparecen físicamente
    "perdida",
    "MagicDelMundo",
    "OtroIdioma",
    # "Proxy",           # descomenta si tampoco quieres verlas
]

# Pendientes de comprar: van a la página "Cartas que busco".
BINDERS_BUSCADAS = ["Comprar", "BUY"]

# Si la detección del comandante queda en duda, preguntar en consola y guardar
# la respuesta en comandantes.txt. Con --sin-preguntar se desactiva (útil para
# ejecuciones automáticas o programadas).
PREGUNTAR_COMANDANTE = True
...
# Texto bajo el banner. Déjalo vacío ("") si no quieres subtítulo.
SUBTITULO = "Colección de Magic: The Gathering"
...
CSV_EXTRA_COLUMNS = [
    ...
    "purchase_price", "binder_name", "precio_usd", "precio_scg",
]
...
COLUMN_WIDTHS = {
    ...
    "purchase_price": 14, "binder_name": 18, "precio_usd": 12, "precio_scg": 12,
}
...
CSV_FIELD_ALIASES = {
    ...
    "added": "added", "dateadded": "added", "fecha": "added",
...
# ── Cartas que busco: proxies y lista de deseos ───────────────────────────────
ARCHIVO_BUSCADAS = "buscadas.txt"


def parse_buscadas(ruta: Path) -> list[dict]:
    """
    Lee buscadas.txt: cartas que quieres conseguir.

    Sirve para dos cosas a la vez:
      · Declarar los PROXIES que hay en tus mazos, para que la lista del mazo
        cuadre a 100 aunque esas cartas no estén escaneadas en Manabox.
      · Llevar una lista de deseos suelta, sin mazo asociado.

    Formato:

        # Sin sección: cartas que buscas en general
        2 Rhystic Study
        Smothering Tithe

        [EDH Urza]                  <- el nombre exacto del binder del mazo
        Mana Drain
        1 Cyclonic Rift (BRO) 62    <- edición y número si quieres una concreta

    Las cartas bajo una sección se consideran proxies de ese mazo: aparecen en
    la lista del mazo con su marca y también en la página de búsqueda.
    """
    entradas = []
    if not ruta.exists():
        return entradas
    ...
    patron = re.compile(
        ...
        r'(?:\s+\((?P<set>[A-Za-z0-9]{2,6})\)(?:\s+(?P<cn>[\w\-\*]+))?)?\s*$')
    ...
    for linea in ruta.read_text(encoding="utf-8").splitlines():
        ...
        entradas.append({
            ...
            "set_name": "",
            "collector_number": m.group("cn") or "",
            "condition": "",
            "language": "",
            "purchase_price": "",
            "binder_name": seccion,      # el mazo al que pertenece, si lo hay
            "added": "",
            "buscada": True,             # marca que la distingue del inventario
            "proxy": bool(seccion),      # con sección = proxy dentro de un mazo
        })
    return entradas
...
# ── Último destape ────────────────────────────────────────────────────────────
ARCHIVO_DESTAPE = "destape.txt"


def parse_destape(ruta: Path) -> dict:
    """
    Lee destape.txt: qué ediciones mostrar en la página del último destape.

        # Prerelease de marzo
        TITULO = Prerelease Tarkir: Dragonstorm
        TDM
        FIN
        DFT

    Una sigla por línea. TITULO es opcional. Al cambiar las siglas y regenerar,
    la página muestra el destape nuevo sin tocar nada más.
    """
    cfg = {"titulo": "", "sets": []}
    if not ruta.exists():
        return cfg
    for linea in ruta.read_text(encoding="utf-8").splitlines():
        ...
        if not linea or linea.startswith("#"):
            continue
        if "=" in linea and linea.split("=", 1)[0].strip().lower() in ("titulo", "título"):
            cfg["titulo"] = linea.split("=", 1)[1].strip()
            continue
        for sigla in re.split(r"[,\s]+", linea):
            sigla = sigla.strip().upper()
            if sigla and sigla not in cfg["sets"]:
                cfg["sets"].append(sigla)
    return cfg


def exportar_destape(rows_todas: list[dict], destino: Path, cfg: dict,
                     sheet_for=None) -> int:
    """
    Escribe data/destape.json con las cartas de las ediciones indicadas.

    Mira TODAS las cartas, también las que ya metiste en un mazo: si abriste un
    sobre y la carta acabó jugándose, sigue siendo parte de ese destape y se
    marca como tal.
    """
    if not cfg.get("sets"):
        return 0
    ...
    siglas = set(cfg["sets"])
    cartas, set_nombres = [], {}
    por_rareza = defaultdict(int)
    total = foils = 0
    valor_destape = 0.0

    for r in rows_todas:
        code = (r.get("set_code") or "").upper()
        if code not in siglas:
            continue
        q = r.get("qty", 1)
        q = q if isinstance(q, int) else 1
        if r.get("set_name"):
            set_nombres.setdefault(code, r["set_name"])

        cid = (r.get("scryfall_id") or "").strip()
        img = (r.get("image_uri", "") or "").split("?", 1)[0]
        if not cid and img:
            t = img.rsplit("/", 1)[-1]
            if t.endswith(".jpg"):
                cid = t[:-4]

        binder = (r.get("binder_name") or "").strip()
        cmc = r.get("cmc", "")
        try:
            precio_r = float(r.get("precio_usd") or 0) or None
        except (TypeError, ValueError):
            precio_r = None
        valor_destape += (precio_r or 0) * q
        cartas.append({
            "id": cid, "n": r.get("name", ""), "tl": r.get("type_line", ""),
            "mc": r.get("mana_cost", ""), "c": cmc if isinstance(cmc, (int, float)) else None,
            "r": r.get("rarity", ""), "s": code,
            "cn": r.get("collector_number", ""), "ci": r.get("color_identity", ""),
            "f": 1 if r.get("foil") else 0, "q": q,
            "t": sheet_for(r) if sheet_for else "Other",
            "p": precio_r,
            "mazo": (binder[len(PREFIJO_MAZO):].strip(" -_")
                     if binder.upper().startswith(PREFIJO_MAZO.upper()) else ""),
        })
        por_rareza[r.get("rarity", "")] += q
        total += q
        if r.get("foil"):
            foils += q

    orden_r = {"mythic": 0, "rare": 1, "special": 2, "bonus": 3, "uncommon": 4, "common": 5}
    cartas.sort(key=lambda c: (orden_r.get(c["r"], 9), -(c["f"]), c["n"]))

    doc = {
        "schema_version": SCHEMA_VERSION,
        "generated_at": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "titulo": cfg.get("titulo", ""),
        "sets": [[c, set_nombres.get(c, "")] for c in cfg["sets"]],
        "totales": {"cartas": total, "distintas": len(cartas), "foils": foils,
                    "miticas": por_rareza.get("mythic", 0),
                    "raras": por_rareza.get("rare", 0),
                    "valor_usd": round(valor_destape, 2)},
        "cartas": cartas,
    }
    data_dir = destino / "data"
    data_dir.mkdir(parents=True, exist_ok=True)
    ruta = data_dir / "destape.json"
    with open(ruta, "w", encoding="utf-8") as f:
        json.dump(doc, f, ensure_ascii=False, separators=(",", ":"))

    print(f"    destape.json    {ruta.stat().st_size/1024:>8.0f} KB "
          f"({total} cartas de {', '.join(cfg['sets'])}: "
          f"{por_rareza.get('mythic',0)} míticas, {por_rareza.get('rare',0)} raras, "
          f"{foils} foils · valor ${valor_destape:,.2f})")
    return total


def exportar_buscadas(rows_buscadas: list[dict], rows_coleccion: list[dict],
                      destino: Path, sheet_for=None) -> int:
    """
    Escribe data/buscadas.json con todo lo que quieres conseguir.

    Para cada carta indica si YA tienes copias en la colección: es la diferencia
    entre "no la tengo" y "la tengo pero necesito otra para este mazo", que son
    dos situaciones muy distintas a la hora de cambiar o comprar.
    """
    if not rows_buscadas:
        return 0

    tengo = defaultdict(int)
    for r in rows_coleccion:
        q = r.get("qty", 1)
        tengo[r.get("name", "").lower()] += q if isinstance(q, int) else 1

    cartas = []
    for r in rows_buscadas:
        cid = (r.get("scryfall_id") or "").strip()
        img = (r.get("image_uri", "") or "").split("?", 1)[0]
        if not cid and img:
            t = img.rsplit("/", 1)[-1]
            if t.endswith(".jpg"):
                cid = t[:-4]
        cmc = r.get("cmc", "")
        mazo = (r.get("binder_name") or "").strip()
        cartas.append({
            "id": cid, "n": r.get("name", ""), "tl": r.get("type_line", ""),
            "mc": r.get("mana_cost", ""), "c": cmc if isinstance(cmc, (int, float)) else None,
            "r": r.get("rarity", ""), "s": r.get("set_code", ""),
            "cn": r.get("collector_number", ""), "ci": r.get("color_identity", ""),
            "q": r.get("qty", 1),
            "t": sheet_for(r) if sheet_for else "Other",
            "mazo": mazo[len(PREFIJO_MAZO):].strip(" -_") if mazo.upper().startswith(PREFIJO_MAZO.upper()) else mazo,
            "px": 1 if r.get("proxy") else 0,
            "p": (lambda v: v if v else None)(
                 (lambda x: round(float(x), 2) if x else None)(r.get("precio_usd") or 0)),
            "tengo": tengo.get(r.get("name", "").lower(), 0),
        })

    cartas.sort(key=lambda c: (c["mazo"] or "zzz", c["n"]))
    ...
    data_dir = destino / "data"
    data_dir.mkdir(parents=True, exist_ok=True)
    ruta = data_dir / "buscadas.json"
    with open(ruta, "w", encoding="utf-8") as f:
        json.dump(doc, f, ensure_ascii=False, separators=(",", ":"))

    proxies = sum(1 for c in cartas if c["px"])
    print(f"    buscadas.json   {ruta.stat().st_size/1024:>8.0f} KB "
          f"({len(cartas)} cartas: {proxies} proxies en mazos, "
          f"{len(cartas)-proxies} de la lista de deseos)")
    return len(cartas)
...
def fetch_by_id(card_id: str) -> dict | None:
    """Consulta una carta por su UUID: devuelve LA impresión exacta."""
    r = _peticion("GET", f"https://api.scryfall.com/cards/{card_id}",
                  f"carta {card_id[:8]}")
    if r is None:
        return None
    try:
...
def fetch_by_set_cn(set_code: str, cn: str) -> dict | None:
    """Consulta por edición y número de coleccionista: también exacta."""
    r = _peticion("GET", f"https://api.scryfall.com/cards/{set_code.lower()}/{cn}",
                  f"carta {set_code}/{cn}")
    if r is None:
        return None
    try:
...
def fetch_preciso(entry: dict) -> dict | None:
    """
    Recupera una carta usando el identificador MÁS preciso disponible.

    Importante: la búsqueda difusa por nombre se deja como último recurso
    porque devuelve la impresión canónica, que puede NO ser la que el jugador
    tiene físicamente (otra edición, otra rareza, otra imagen). Preferimos
    siempre el UUID o set+número, que identifican la copia exacta.
    """
    ...
    if set_code and cn:
        d = fetch_by_set_cn(set_code, cn)
        if d:
            return d
    return fetch_scryfall(entry["name"])
...
def enrich(entry: dict, data: dict | None, columns: list[str] | None = None) -> dict:
    ...
    # Marcas internas que no son columnas del Excel pero el resto del pipeline
    # necesita: sin ellas los proxies se confunden con inventario real y el
    # desempate del comandante por fecha se queda sin dato
    for meta in ("buscada", "proxy", "added", "scryfall_id", "precio_usd"):
        if entry.get(meta):
            row[meta] = entry[meta]
    ...
    # Precio de mercado. Scryfall da usd / usd_foil / usd_etched; elegimos
    # según el acabado de la copia física. Si falta, cae al de Manabox.
    precios = data.get("prices") or {}
    if entry.get("foil"):
        bruto = precios.get("usd_foil") or precios.get("usd_etched") or precios.get("usd")
    else:
        bruto = precios.get("usd") or precios.get("usd_foil")
    try:
        row["precio_usd"] = round(float(bruto), 2) if bruto else ""
    except (TypeError, ValueError):
        row["precio_usd"] = ""
    # Precio de SCG, si hay CSV del scraper cargado
    p_scg = precio_scg(row["name"], data.get("set", ""),
                       data.get("collector_number", ""), entry.get("foil"))
    row["precio_scg"] = p_scg if p_scg else ""
    if FUENTE_PRECIO == "scg" and p_scg:
        row["precio_usd"] = round(p_scg, 2)

    if not row["precio_usd"] and entry.get("purchase_price"):
        try:
            row["precio_usd"] = round(float(entry["purchase_price"]), 2)
        except (TypeError, ValueError):
            pass
...
# ══════════════════════════════════════════════════════════════════════════════
# CACHÉ LOCAL DE SCRYFALL (SQLite)
# ══════════════════════════════════════════════════════════════════════════════
#
# Los datos de una impresión concreta son inmutables: el oracle text, el tipo,
# el coste y la rareza de un Sol Ring de LTC no van a cambiar nunca. Volver a
# pedirlos a Scryfall en cada ejecución es tiempo perdido y carga innecesaria
# para un servicio gratuito.
#
# Esta caché guarda cada carta indexada por su UUID de Scryfall, y además
# registra alias (nombre, o set+número) para poder resolver decklists de TXT
# que no traen el UUID.
#
# Lo ÚNICO que cambia con el tiempo son los precios, así que se guarda la fecha
# de cacheo y se puede forzar el refresco con --refrescar.

CACHE_FILE = "scryfall_cache.sqlite"
USAR_CACHE = True        # lo desactiva --sin-cache
REFRESCAR_CACHE = False  # lo activa --refrescar

# Campos que conservamos. Guardar la respuesta íntegra de Scryfall multiplicaría
# por cinco el tamaño con datos que no usamos (rulings, enlaces de compra...).
CAMPOS_CACHE = (
    "id", "oracle_id", "name", "lang", "released_at", "scryfall_uri", "layout",
    "mana_cost", "cmc", "type_line", "oracle_text", "power", "toughness",
    "loyalty", "colors", "color_identity", "keywords", "set", "set_name",
    "collector_number", "rarity", "flavor_text", "prices", "edhrec_rank",
    "digital", "reserved",
)
CAMPOS_CARA = (
    "name", "mana_cost", "type_line", "oracle_text", "power", "toughness",
    "loyalty", "colors", "flavor_text", "image_uris",
)


def podar(data: dict) -> dict:
    """Se queda con lo que usamos. De las imágenes solo la 'normal': las demás
    resoluciones se derivan de la URL."""
    if not data:
        return data
    out = {k: data[k] for k in CAMPOS_CACHE if k in data}
    if "image_uris" in data and isinstance(data["image_uris"], dict):
        normal = data["image_uris"].get("normal", "")
        if normal:
            out["image_uris"] = {"normal": normal}
    if "card_faces" in data:
        caras = []
        for cara in data["card_faces"]:
            c = {k: cara[k] for k in CAMPOS_CARA if k in cara}
            if "image_uris" in c and isinstance(c["image_uris"], dict):
                n = c["image_uris"].get("normal", "")
                c["image_uris"] = {"normal": n} if n else {}
            caras.append(c)
        out["card_faces"] = caras
    return out


def cache_abrir(carpeta: Path):
    """Abre (o crea) la base de datos junto al script."""
    if not USAR_CACHE:
        return None
    try:
        ruta = carpeta / CACHE_FILE
        con = sqlite3.connect(str(ruta))
        con.execute("""CREATE TABLE IF NOT EXISTS cartas (
                         id TEXT PRIMARY KEY,
                         nombre TEXT,
                         set_code TEXT,
                         cn TEXT,
        ...
        con.execute("""CREATE TABLE IF NOT EXISTS alias (
                         ...
                         id TEXT)""")
        con.execute("CREATE INDEX IF NOT EXISTS ix_nombre ON cartas(nombre)")
        con.commit()
        return con
    except Exception as e:
        print(f"  ⚠ No se pudo abrir la caché ({e}). Se seguirá sin ella.")
        return None


def cache_buscar(con, claves: list[str]) -> dict:
    """Devuelve {clave: data} para las claves que estén en la caché."""
    if con is None or REFRESCAR_CACHE or not claves:
        return {}
    encontrados = {}
    try:
        cur = con.cursor()
        for i in range(0, len(claves), 400):     # SQLite limita los parámetros
            lote = claves[i:i + 400]
            marcas = ",".join("?" * len(lote))

            # Claves directas por UUID
            ids = [k[3:] for k in lote if k.startswith("id:")]
            if ids:
                m = ",".join("?" * len(ids))
                for cid, data in cur.execute(
                        f"SELECT id, data FROM cartas WHERE id IN ({m})", ids):
                    encontrados["id:" + cid] = json.loads(data)

            # Claves indirectas (name:..., cn:...) vía tabla de alias
            otras = [k for k in lote if not k.startswith("id:")]
            if otras:
                m = ",".join("?" * len(otras))
                filas = cur.execute(
                    f"""SELECT a.clave, c.data FROM alias a
                        JOIN cartas c ON c.id = a.id
                        WHERE a.clave IN ({m})""", otras).fetchall()
                for clave, data in filas:
                    encontrados[clave] = json.loads(data)
    except Exception as e:
        print(f"  ⚠ Error leyendo la caché ({e}). Se descargará todo.")
        return {}
    return encontrados


def cache_guardar(con, pares: list[tuple]):
    """pares = [(clave_original, data), ...]"""
    if con is None or not pares:
        return
    ahora = datetime.now(timezone.utc).isoformat(timespec="seconds")
    try:
        cur = con.cursor()
        for clave, data in pares:
            if not data or not data.get("id"):
                continue
            recorte = podar(data)
            cur.execute(
                "INSERT OR REPLACE INTO cartas (id,nombre,set_code,cn,data,cached_at)"
                " VALUES (?,?,?,?,?,?)",
                (data["id"], data.get("name", ""), data.get("set", ""),
                 data.get("collector_number", ""),
                 json.dumps(recorte, ensure_ascii=False), ahora))

            # Alias para poder resolver por nombre o por set+número
            alias = {clave, f"name:{data.get('name','').lower()}"}
            if data.get("set") and data.get("collector_number"):
                alias.add(f"cn:{data['set'].lower()}:{data['collector_number']}")
            for a in alias:
                if a and not a.startswith("id:"):
                    cur.execute("INSERT OR REPLACE INTO alias (clave,id) VALUES (?,?)",
                                (a, data["id"]))
        con.commit()
    except Exception as e:
        print(f"  ⚠ Error escribiendo en la caché ({e}).")


def cache_stats(con) -> int:
    if con is None:
        return 0
    try:
        return con.execute("SELECT COUNT(*) FROM cartas").fetchone()[0]
    except Exception:
        return 0
...
def build_cache(uniques: dict) -> dict:
    ...
    # ── Paso 1: lo que ya tenemos en la caché local ───────────────────────────
    con = cache_abrir(Path(__file__).parent)
    guardadas = cache_stats(con)
    desde_cache = cache_buscar(con, list(pending.keys()))
    for clave, data in desde_cache.items():
        cache[clave] = data
        pending.pop(clave, None)

    if desde_cache:
        print(f"  Caché local: {len(desde_cache)} carta(s) recuperadas "
              f"({guardadas} en la base)")
        # Los datos de una impresión no caducan, pero los precios sí
        try:
            fila = con.execute("SELECT MIN(cached_at) FROM cartas").fetchone()
            if fila and fila[0]:
                dias = (datetime.now(timezone.utc)
                        - datetime.fromisoformat(fila[0])).days
                if dias >= 14:
                    print(f"  ⚠ Los precios más antiguos tienen {dias} días. "
                          f"Usa --precios para actualizarlos.")
        except Exception:
            pass
    if not pending:
        print("  Nada que descargar: todo estaba en caché.")
        if con:
            con.close()
        return cache

    # ── Paso 2: solo lo que falta se pide a Scryfall ──────────────────────────
    keys        = list(pending.keys())
    identifiers = [build_identifier(pending[k]) for k in keys]

    print(f"  Faltan {len(keys)} carta(s). Consultando Scryfall por lotes de {BATCH_SIZE}...")
    ...
    nuevas = [(k, cache[k]) for k in keys if cache.get(k)]

    # ── Reintentos ────────────────────────────────────────────────────────────
    # Ronda 1: repetir el lote completo. La mayoría de fallos son transitorios
    #          (saturación momentánea), no cartas inexistentes.
    if faltantes:
        print(f"  Reintentando {len(faltantes)} carta(s) en lote...")
        time.sleep(1.0)
        ids2 = [build_identifier(pending[k]) for k in faltantes]
        found2, _ = fetch_scryfall_batch(ids2)
        by_id2 = {d["id"]: d for d in found2 if d.get("id")}
        by_cn2 = {f"{d['set'].lower()}:{d['collector_number']}": d
                  for d in found2 if d.get("set") and d.get("collector_number")}
        by_nm2 = {}
        for d in found2:
            by_nm2.setdefault(d.get("name", "").lower(), d)

        aun_faltan = []
        for key in faltantes:
            ident = build_identifier(pending[key])
            data = (by_id2.get(ident.get("id")) if "id" in ident else
                    by_cn2.get(f"{ident.get('set')}:{ident.get('collector_number')}")
                    if "set" in ident else
                    by_nm2.get(pending[key]["name"].lower()))
            if data:
                cache[key] = data
                nuevas.append((key, data))
            else:
                aun_faltan.append(key)
        faltantes = aun_faltan

    # Ronda 2: una a una, con el identificador más preciso que tengamos
    if faltantes:
        print(f"  Reintento individual de {len(faltantes)} carta(s)...")
        for key in faltantes:
            entry = pending[key]
            data  = fetch_preciso(entry)
            cache[key] = data
            if data:
                nuevas.append((key, data))
            else:
                print(f"    - {entry['name']}: NO ENCONTRADA")
            time.sleep(REQUEST_DELAY * 2)

    # ── Paso 3: guardar lo nuevo para la próxima vez ──────────────────────────
    if nuevas and con:
        cache_guardar(con, nuevas)
        print(f"  Caché actualizada: +{len(nuevas)} carta(s) "
              f"({cache_stats(con)} en total)")
    if con:
        con.close()
...
# ============================================================================
# PRECIOS DE STARCITYGAMES
# ============================================================================
#
# scrape_all_sets.py genera un CSV con los listados de SCG. Este módulo lo lee y
# lo cruza con tu colección. Los precios de SCG son de tienda (venta al público
# en EE.UU.); los de Scryfall son de mercado (TCGplayer). Suelen diferir, así
# que se guardan los dos y tú eliges cuál se publica.

ARCHIVO_PRECIOS_SCG = "precios_scg.csv"

# Qué precio se publica en la web y alimenta los totales:
#   "scg"      → el de StarCityGames, con Scryfall de respaldo
#   "scryfall" → siempre el de Scryfall
FUENTE_PRECIO = "scg"

SCG_CONDICION = "near mint"     # condición de referencia
SCG_IDIOMA    = "english"       # idioma de referencia
PRECIOS_SCG   = {}              # lo llena cargar_precios_scg()


def _norm(t: str) -> str:
    """Normaliza nombres para comparar: sin acentos, signos ni mayúsculas."""
    t = (t or "").lower().strip()
...
def mapa_sets_scryfall(con) -> dict:
    """
    {nombre_de_edicion_normalizado: codigo}. Hace falta porque el CSV de SCG
    trae el nombre de la edición ("The Brothers' War") y aquí trabajamos con el
    código ("BRO"). Se descarga una vez y queda guardado en la caché.
    """
    mapa = {}
    if con is not None:
        try:
            con.execute("CREATE TABLE IF NOT EXISTS sets_scryfall (nombre TEXT PRIMARY KEY, code TEXT)")
            for nombre, code in con.execute("SELECT nombre, code FROM sets_scryfall"):
                mapa[nombre] = code
            con.commit()
        except Exception:
            pass
    ...
    r = _peticion("GET", "https://api.scryfall.com/sets", "lista de ediciones")
    if r is None:
        return mapa
    try:
        for s in r.json().get("data", []):
            if s.get("name") and s.get("code"):
                mapa[_norm(s["name"])] = s["code"].upper()
    except Exception:
        return mapa

    if con is not None and mapa:
        try:
            con.executemany("INSERT OR REPLACE INTO sets_scryfall VALUES (?,?)", list(mapa.items()))
            con.commit()
            print(f"    ediciones de Scryfall cacheadas: {len(mapa)}")
        except Exception:
            pass
    return mapa


def cargar_precios_scg(ruta: Path, con=None) -> dict:
    """
    Lee el CSV del scraper y devuelve un índice de precios.

    Se generan tres claves por carta, de la más precisa a la más laxa, para que
    siempre haya con qué casar:
        "SET|123|1"    edición + número + foil   (exacta)
        "nombre|123|1" nombre + número + foil    (si no se mapeó la edición)
        "nombre|1"     nombre + foil             (último recurso)
    """
    global PRECIOS_SCG
    PRECIOS_SCG = {}
    if not ruta.exists():
    ...
    mapa_sets = mapa_sets_scryfall(con)
    filas = descartadas = 0

    def anota(clave, precio):
        # Nos quedamos con el listado más barato de esa impresión
        if clave and (clave not in PRECIOS_SCG or precio < PRECIOS_SCG[clave]):
            PRECIOS_SCG[clave] = precio

    with open(ruta, encoding="utf-8-sig", newline="") as f:
        for fila in csv.DictReader(f):
            cond = (fila.get("condition") or "").lower()
            idioma = (fila.get("language") or "").lower()
            if SCG_CONDICION and SCG_CONDICION not in cond:
                descartadas += 1; continue
            if SCG_IDIOMA and idioma and SCG_IDIOMA not in idioma:
                descartadas += 1; continue
            try:
                precio = float(str(fila.get("price_usd") or "").replace("$", "").replace(",", ""))
            except ValueError:
                descartadas += 1; continue
            if precio <= 0:
                continue

            nombre = _norm(fila.get("card_name"))
            cn = (fila.get("collector_number") or "").strip()
            acabado = (fila.get("finish") or "").lower()
            foil = 1 if ("foil" in acabado and "non" not in acabado) else 0
            code = mapa_sets.get(_norm(fila.get("set_name")), "")

            if code and cn:
                anota(f"{code}|{cn}|{foil}", precio)
            if nombre and cn:
                anota(f"{nombre}|{cn}|{foil}", precio)
            if nombre:
                anota(f"{nombre}|{foil}", precio)
            filas += 1

    print(f"    precios SCG: {filas} listados de {ruta.name} "
          f"({descartadas} descartados por condición o idioma)")
    return PRECIOS_SCG


def precio_scg(nombre: str, set_code: str, cn: str, foil) -> float | None:
    """Busca el precio de SCG de la forma más precisa posible."""
    if not PRECIOS_SCG:
        return None
    f = 1 if foil else 0
    n = _norm(nombre)
    cn = (cn or "").strip()
    for clave in (f"{(set_code or '').upper()}|{cn}|{f}", f"{n}|{cn}|{f}", f"{n}|{f}"):
        if clave in PRECIOS_SCG:
            return PRECIOS_SCG[clave]
    return None
...
def actualizar_version_assets(index_html: Path, version: str):
    """
    Reescribe assets/app.js?v=... y styles.css?v=... con la fecha de generación.

    Sin esto, los navegadores siguen usando la copia guardada del CSS y del JS
    durante días: tú publicas cambios y quien ya visitó el sitio sigue viendo lo
    de antes. Cambiar el número basta para que lo consideren un archivo nuevo.
    """
    if not index_html.exists():
        return
    try:
        html = index_html.read_text(encoding="utf-8")
        nuevo = re.sub(r'(assets/(?:app\.js|mazos\.js|buscadas\.js|constructor\.js|destape\.js|sld\.js|styles\.css))(\?v=[^"\']*)?',
                       lambda m: f"{m.group(1)}?v={version}", html)
        if nuevo != html:
            index_html.write_text(nuevo, encoding="utf-8")
            print(f"    versión de assets actualizada: v={version}")
    except Exception as e:
        print(f"    ⚠ No se pudo actualizar la versión en index.html: {e}")


SIMBOLOGIA_URL = "https://api.scryfall.com/symbology"


def descargar_simbolos(carpeta: Path) -> dict:
    """
    Baja los símbolos de maná de Scryfall en SVG y devuelve {símbolo: archivo}.

    Son ~90 vectores minúsculos que cubren todo: básicos, genéricos, híbridos,
    Phyrexian, tap, energía... Al ser SVG se ven nítidos a cualquier tamaño y
    en cualquier pantalla, cosa que un PNG de 128px no puede dar.

    Solo se descargan una vez: si el archivo ya existe en la carpeta, se salta.
    """
    carpeta.mkdir(parents=True, exist_ok=True)
    mapa = {}

    r = _peticion("GET", SIMBOLOGIA_URL, "simbología")
    if r is None:
        # Sin conexión reutilizamos lo que ya esté descargado
        for svg in carpeta.glob("*.svg"):
            mapa["{" + svg.stem + "}"] = svg.name
        if mapa:
            print(f"    símbolos: usando los {len(mapa)} ya descargados")
        return mapa
    ...
    nuevos = 0
    for sim in datos:
        simbolo = sim.get("symbol", "")
        uri = sim.get("svg_uri", "")
        if not simbolo or not uri:
            continue
        archivo = uri.rsplit("/", 1)[-1]          # "2W.svg", "WP.svg", "T.svg"
        destino_svg = carpeta / archivo
        mapa[simbolo] = archivo
        if destino_svg.exists():
            continue
        rr = _peticion("GET", uri, f"símbolo {simbolo}")
        if rr is not None and rr.content:
            destino_svg.write_bytes(rr.content)
            nuevos += 1
            time.sleep(0.05)

    total_kb = sum(f.stat().st_size for f in carpeta.glob("*.svg")) / 1024
    print(f"    símbolos: {len(mapa)} disponibles "
          f"({nuevos} nuevos, {total_kb:.0f} KB en total)")
    return mapa



def _empieza_por(binder: str, prefijos: list) -> bool:
    b = (binder or "").strip().lower()
    return bool(b) and any(b.startswith(p.strip().lower()) for p in prefijos)


def es_exhibicion(row: dict) -> bool:
    """¿Esta carta va a la vitrina en vez de al catálogo de intercambio?"""
    return _empieza_por(row.get("binder_name"), BINDERS_EXHIBICION)


def es_oculta(row: dict) -> bool:
    """Binders que no se publican: jumpstart, perdidas, otro idioma..."""
    return _empieza_por(row.get("binder_name"), BINDERS_OCULTOS)


def es_por_comprar(row: dict) -> bool:
    """Binders que representan cartas que quieres conseguir."""
    return _empieza_por(row.get("binder_name"), BINDERS_BUSCADAS)


def clasificar(row: dict) -> str:
    """
    Decide dónde va cada carta. El orden importa: una carta marcada en
    buscadas.txt manda sobre su binder, y un binder oculto sobre todo lo demás.
    """
    if row.get("buscada"):
        return "buscada"
    if es_oculta(row):
        return "oculta"
    if es_por_comprar(row):
        return "buscada"
    if es_de_mazo(row):
        return "mazo"
    if es_exhibicion(row):
        return "vitrina"
    return "catalogo"


def es_de_mazo(row: dict) -> bool:
    """¿Esta carta pertenece a un mazo armado en vez de a la carpeta de cambio?"""
    binder = (row.get("binder_name") or "").strip()
    return bool(PREFIJO_MAZO) and binder.upper().startswith(PREFIJO_MAZO.upper())


def slug(texto: str) -> str:
    """'EDH Gishath, Sun's Avatar' → 'gishath-suns-avatar'"""
    t = texto.strip()
    if t.upper().startswith(PREFIJO_MAZO.upper()):
        t = t[len(PREFIJO_MAZO):]
    t = t.strip(" -_")
    reemplazos = {"á":"a","é":"e","í":"i","ó":"o","ú":"u","ñ":"n","ü":"u"}
    t = "".join(reemplazos.get(c, c) for c in t.lower())
    t = re.sub(r"[^a-z0-9]+", "-", t).strip("-")
    return t or "mazo"


ARCHIVO_COMANDANTES = "comandantes.txt"


def cargar_overrides_comandante(carpeta: Path) -> dict:
    """
    Lee comandantes.txt para forzar el comandante de un mazo concreto.

    Formato, una línea por mazo (se ignoran vacías y las que empiezan por #):

        EDH Urza = Urza, Lord Protector
        EDH Atraxa = Atraxa, Praetors' Voice

    La detección automática acierta casi siempre, pero cuando hay varias
    legendarias con el mismo nombre corto (Urza, Jace, Teferi...) conviene
    poder decirlo a mano en vez de pelearse con heurísticas.
    """
    ruta = carpeta / ARCHIVO_COMANDANTES
    overrides = {}
    if not ruta.exists():
        return overrides
    try:
        for linea in ruta.read_text(encoding="utf-8").splitlines():
            ...
            if not linea or linea.startswith("#") or "=" not in linea:
                continue
            binder, carta = linea.split("=", 1)
            overrides[binder.strip().lower()] = carta.strip()
        if overrides:
            print(f"    comandantes fijados a mano: {len(overrides)}")
    except Exception as e:
        print(f"    ⚠ No se pudo leer {ARCHIVO_COMANDANTES}: {e}")
    return overrides


def puede_ser_comandante(carta: dict) -> bool:
    tl = carta.get("type_line") or ""
    txt = (carta.get("oracle_text") or "").lower()
    return (("Legendary" in tl and "Creature" in tl)
            or "can be your commander" in txt
            or "Background" in tl)


def es_socio(carta: dict) -> bool:
    """
    ¿Esta carta puede compartir el mando con otra?

    Cubre todas las variantes que permiten dos comandantes:
      · Partner y Partner with          (Akiri + Ardenn)
      · Friends forever                 (Stranger Things)
      · Doctor's companion              (Doctor Who)
      · Choose a Background + Background (Baldur's Gate)
    """
    tl = carta.get("type_line") or ""
    txt = (carta.get("oracle_text") or "").lower()
    return ("partner" in txt or "friends forever" in txt
            or "doctor's companion" in txt or "choose a background" in txt
            or "Background" in tl)


def detectar_comandante(nombre_mazo: str, cartas: list[dict], override: str = ""):
    """
    Devuelve (comandantes, identidad, aviso).

    El criterio decisivo es la regla real del formato: la identidad de color del
    comandante debe CONTENER la de todas las cartas del mazo. Eso descarta solo
    los casos ambiguos por nombre — un Urza mono azul no puede comandar un mazo
    con cartas blancas, por mucho que el binder se llame "EDH Urza".

    Orden: anulación manual → único candidato legal → coincidencia de nombre
    entre los legales → pareja de socios que entre los dos cubran la identidad.
    """
    candidatos = [c for c in cartas if puede_ser_comandante(c)]

    union = set()
    for c in cartas:
        union.update(c.get("color_identity", "") or "")
    identidad_mazo = "".join(x for x in "WUBRG" if x in union)

    # 1. Anulación manual
    if override:
        for c in candidatos:
            if c["name"].lower() == override.lower():
                return [c], c.get("color_identity", ""), ""
        return [], identidad_mazo, f"'{override}' no está entre las cartas del mazo"

    if not candidatos:
        return [], identidad_mazo, "sin legendarias que puedan ser comandante"

    # 2. ¿Mazo de socios? Akiri + Ardenn, Doctor + companion, comandante +
    #    Background... Hay que mirarlo ANTES de buscar comandante único, porque
    #    muchas veces uno de los dos ya cubre la identidad él solo y el otro
    #    pasaría por una carta más del mazo.
    base = nombre_mazo.lower()
    socios = [c for c in candidatos if es_socio(c)]

    def combinada(a, b):
        j = set(a.get("color_identity", "") or "") | set(b.get("color_identity", "") or "")
        return "".join(x for x in "WUBRG" if x in j), j

    # 2a. El binder nombra a los dos por su nombre COMPLETO. Exigimos nombre
    #     completo porque con el corto "EDH Urza" coincidirían los dos Urzas
    #     del mazo y los tomaría por socios, que fue justo el error anterior.
    nombrados = [c for c in candidatos if c["name"].lower() in base]
    if len(nombrados) == 2:
        ident, juntos = combinada(nombrados[0], nombrados[1])
        if union <= juntos:
            return nombrados, ident, ""

    # 2b. Exactamente dos cartas con Partner (o equivalente): son la pareja
    if len(socios) == 2:
        ident, juntos = combinada(socios[0], socios[1])
        if union <= juntos:
            return socios, ident, ""

    # 2c. Más de dos socios: nos quedamos con los que nombre el binder
    if len(socios) > 2:
        elegidos = [c for c in socios
                    if c["name"].lower() in base
                    or c["name"].split(",")[0].strip().lower() in base]
        if len(elegidos) == 2:
            ident, _ = combinada(elegidos[0], elegidos[1])
            return elegidos, ident, ""

    # 2. Solo los que cubren la identidad de todo el mazo
    legales = [c for c in candidatos
               if union <= set(c.get("color_identity", "") or "")]

    if len(legales) == 1:
        return legales, legales[0].get("color_identity", ""), ""

    if legales:
        base = nombre_mazo.lower()

        # 3a. Nombre COMPLETO en el binder ("EDH Urza, Lord Protector"):
        #     es la señal más fuerte que existe, no hay ambigüedad posible
        exactos = [c for c in legales if c["name"].lower() in base]
        if len(exactos) == 1:
            return exactos, exactos[0].get("color_identity", ""), ""
        if len(exactos) > 1:
            legales = exactos

        # 3b. Nombre corto ("EDH Urza"): puede coincidir con varias cartas
        cortos = [c for c in legales
                  if c["name"].split(",")[0].strip().lower() in base]
        if len(cortos) == 1:
            return cortos, cortos[0].get("color_identity", ""), ""
        if cortos:
            legales = cortos
        ...
        # 3c. Desempate por fecha: al armar un mazo en Manabox lo normal es
        #     meter el comandante primero, así que la carta más antigua del
        #     binder es la candidata más probable
        con_fecha = [c for c in legales if c.get("added")]
        if con_fecha:
            con_fecha.sort(key=lambda c: c["added"])
            elegido = con_fecha[0]
            return [elegido], elegido.get("color_identity", ""), \
                   (f"{len(legales)} candidatos; elegido el primero añadido al binder "
                    f"({', '.join(c['name'] for c in legales)})")

        elegido = legales[0]
        return [elegido], elegido.get("color_identity", ""), \
               f"{len(legales)} candidatos: {', '.join(c['name'] for c in legales)}"

    # 5. Última red: nadie cubre la identidad en solitario. Solo emparejamos si
    #    ambas cartas son socios de verdad; si no, lo más probable es que haya
    #    una carta fuera de la identidad del mazo y hay que avisar, no inventar.
    for i, a in enumerate(candidatos):
        ...
        for b in candidatos[i + 1:]:
            if not es_socio(b):
                continue
            ident, juntos = combinada(a, b)
            if union <= juntos:
                return [a, b], ident, "pareja de socios detectada"

    # Si alguna legendaria coincide con el nombre del mazo, la damos por buena
    # y avisamos: seguramente hay cartas coladas fuera de su identidad
    por_nombre = [c for c in candidatos
                  if c["name"].lower() in base
                  or c["name"].split(",")[0].strip().lower() in base]
    if por_nombre:
        elegido = por_nombre[0]
        suyos = set(elegido.get("color_identity", "") or "")
        sobra = "".join(x for x in "WUBRG" if x in (union - suyos))
        return [elegido], elegido.get("color_identity", ""), \
               (f"hay cartas fuera de la identidad de {elegido['name']}: "
                f"colores extra {sobra} — revisa el binder")

    return [], identidad_mazo, ("ninguna legendaria cubre la identidad del mazo "
                                f"({identidad_mazo}) — revisa el binder")


def guardar_override(carpeta: Path, binder: str, carta: str):
    """Añade la elección a comandantes.txt para no volver a preguntar."""
    ruta = carpeta / ARCHIVO_COMANDANTES
    try:
        nuevo = not ruta.exists()
        with open(ruta, "a", encoding="utf-8") as f:
            if nuevo:
                f.write("# Comandante de cada mazo. Formato:  BINDER = Nombre exacto de la carta\n")
            f.write(f"{binder} = {carta}\n")
        print(f"        guardado en {ARCHIVO_COMANDANTES}: no se volverá a preguntar")
    except Exception as e:
        print(f"        ⚠ No se pudo guardar la elección: {e}")


def preguntar_comandante(binder: str, candidatos: list[dict], identidad: str):
    """
    Pregunta cuál es el comandante cuando la detección no lo tiene claro.
    Solo se lanza en modo interactivo y solo una vez por mazo: la respuesta
    queda guardada en comandantes.txt.
    """
    if not candidatos or not sys.stdin or not sys.stdin.isatty() or PREGUNTAR_COMANDANTE is False:
        return None
    print(f"\n    ¿Cuál es el comandante de '{binder}'?  (identidad del mazo: {identidad or '—'})")
    for i, c in enumerate(candidatos[:20], 1):
        ci = c.get("color_identity", "") or "incoloro"
        print(f"      [{i}] {c['name']:<38} [{ci}]")
    print("      [0] Dejarlo sin comandante")
    try:
...
CARPETA_GUIAS = "guias"
EXT_AUDIO = (".mp3", ".m4a", ".ogg", ".wav")
EXT_IMG_GUIA = (".png", ".jpg", ".jpeg", ".webp", ".gif")


def buscar_guia(destino: Path, slug_mazo: str) -> dict:
    """
    Busca material de apoyo del mazo en ColeccionWeb/guias/.

    Basta con nombrar los archivos con el slug del mazo:
        guias/sephiroth.md        → texto de la guía (markdown)
        guias/sephiroth.mp3       → podcast (NotebookLM, por ejemplo)
        guias/sephiroth-1.png     → infografía (varias con -1, -2...)

    No hay que configurar nada: si el archivo está, aparece en la web.
    """
    carpeta = destino / CARPETA_GUIAS
    if not carpeta.is_dir():
        return {}
    guia = {"texto": "", "audio": "", "imagenes": []}
    for archivo in sorted(carpeta.iterdir()):
        if not archivo.is_file():
            continue
        nombre = archivo.stem.lower()
        if nombre != slug_mazo and not nombre.startswith(slug_mazo + "-"):
            continue
        ext = archivo.suffix.lower()
        ruta = f"{CARPETA_GUIAS}/{archivo.name}"
        if ext in (".md", ".txt") and not guia["texto"]:
            guia["texto"] = ruta
        elif ext in EXT_AUDIO and not guia["audio"]:
            guia["audio"] = ruta
        elif ext in EXT_IMG_GUIA:
            guia["imagenes"].append(ruta)
    return guia if (guia["texto"] or guia["audio"] or guia["imagenes"]) else {}


def exportar_mazos(rows_mazo: list[dict], destino: Path, sheet_for=None) -> int:
    """
    Escribe data/mazos.json con todos los mazos armados y sus cartas.

    Un mazo de Commander son ~100 cartas; con veinte mazos el archivo ronda los
    100 KB comprimidos, así que va todo en un único JSON: más simple de generar,
    de cachear y de servir que un archivo por mazo.
    """
    if not rows_mazo:
        return 0

    overrides = cargar_overrides_comandante(Path(__file__).parent)

    # Normalizamos el nombre del binder al agrupar: un espacio de más o una
    # mayúscula distinta partían el mazo en dos, y los proxies acababan en un
    # mazo fantasma en vez de sumarse al de verdad
    def clave_binder(nombre):
        return re.sub(r"\s+", " ", (nombre or "").strip()).lower()

    por_mazo = defaultdict(list)
    nombre_real = {}
    for row in rows_mazo:
        binder = (row.get("binder_name") or "").strip()
        k = clave_binder(binder)
        por_mazo[k].append(row)
        # Para el nombre visible mandan las cartas reales del inventario;
        # buscadas.txt solo aporta la grafía si el mazo fuera todo proxies
        if not row.get("buscada"):
            nombre_real.setdefault(k, binder)
        else:
            nombre_real.setdefault("_px_" + k, binder)

    mazos = []
    for clave, cartas in sorted(por_mazo.items()):
        nombre_binder = nombre_real.get(clave) or nombre_real.get("_px_" + clave) or clave
        nombre = nombre_binder[len(PREFIJO_MAZO):].strip(" -_") or nombre_binder

        comandantes, identidad, aviso = detectar_comandante(
            nombre_binder, cartas, overrides.get(nombre_binder.lower(), ""))

        # Si quedó duda y estamos en una consola, preguntamos UNA vez y lo
        # recordamos: a partir de ahí el mazo queda resuelto para siempre
        if aviso and nombre_binder.lower() not in overrides:
            candidatos = [c for c in cartas if puede_ser_comandante(c)]
            elegido = preguntar_comandante(nombre_binder, candidatos, identidad)
            if elegido is not None:
                comandantes = [elegido]
                identidad = elegido.get("color_identity", "")
                aviso = ""
                guardar_override(Path(__file__).parent, nombre_binder, elegido["name"])

        nombres_cmd = {c["name"] for c in comandantes}

        js_cartas, copias, suma_cmc, con_cmc = [], 0, 0, 0
        valor_mazo = 0.0
        ...
        for c in cartas:
            ...
            cid = (c.get("scryfall_id") or "").strip()
            img = (c.get("image_uri", "") or "").split("?", 1)[0]
            if not cid and img:
                t = img.rsplit("/", 1)[-1]
                if t.endswith(".jpg"):
                    cid = t[:-4]

            tipo = sheet_for(c) if sheet_for else "Other"
            tipos[tipo] += qty
            try:
                precio_c = float(c.get("precio_usd") or 0) or None
            except (TypeError, ValueError):
                precio_c = None
            valor_mazo += (precio_c or 0) * qty

            js_cartas.append({
                "id": cid, "n": c.get("name", ""), "tl": tl,
                "mc": c.get("mana_cost", ""), "c": cmc,
                "r": c.get("rarity", ""), "s": c.get("set_code", ""),
                "cn": c.get("collector_number", ""), "ci": c.get("color_identity", ""),
                "f": 1 if c.get("foil") else 0, "q": qty,
                "t": tipo,
                "pt": (f"{c.get('power','')}/{c.get('toughness','')}"
                       if c.get("power", "") != "" or c.get("toughness", "") != "" else ""),
                "cmd": 1 if c.get("name") in nombres_cmd else 0,
                "px": 1 if c.get("proxy") else 0,
                "p": precio_c,
            })

        js_cartas.sort(key=lambda x: (-x["cmd"], x["t"], x["n"]))

        n_proxies = sum(c["q"] for c in js_cartas if c["px"])

        sl = slug(nombre_binder)
        guia = buscar_guia(destino, sl)
        ...
        mazos.append({
            ...
            "slug": sl,
            "nombre": nombre,
            "binder": nombre_binder,
            "comandante": " + ".join(c["name"] for c in comandantes),
            "portada": (js_cartas[0]["id"] if js_cartas else ""),
            "ci": identidad,
            "aviso": aviso,
            "cartas": len(js_cartas),
            "copias": copias,
            "cmc_medio": round(suma_cmc / con_cmc, 2) if con_cmc else 0,
            "valor_usd": round(valor_mazo, 2),
            "tipos": dict(tipos),
            "lista": js_cartas,
        })
    ...
    doc = {
        "schema_version": SCHEMA_VERSION,
        "generated_at": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "prefijo": PREFIJO_MAZO,
        "mazos": mazos,
    }
    data_dir = destino / "data"
    data_dir.mkdir(parents=True, exist_ok=True)
    ruta = data_dir / "mazos.json"
    with open(ruta, "w", encoding="utf-8") as f:
        json.dump(doc, f, ensure_ascii=False, separators=(",", ":"))

    print(f"    mazos.json      {ruta.stat().st_size/1024:>8.0f} KB "
          f"({len(mazos)} mazos, {sum(m['copias'] for m in mazos)} cartas)")
    dudosos = 0
    for m in mazos:
        etiqueta = f"  ← {m['comandante']}" if m["comandante"] else "  ← SIN COMANDANTE"
        print(f"      · {m['nombre']:<26} {m['copias']:>3} cartas "
              f"[{m['ci'] or '—'}] ${m['valor_usd']:>8,.2f}{etiqueta}")
        if m.get("guia"):
            g = m["guia"]
            partes = [x for x in (
                "texto" if g.get("texto") else "",
                "audio" if g.get("audio") else "",
                f"{len(g['imagenes'])} imagen(es)" if g.get("imagenes") else "") if x]
            print(f"          guía: {', '.join(partes)}")
        if m["aviso"]:
            dudosos += 1
            print(f"          ⚠ {m['aviso']}")
    if dudosos:
        print(f"\n      Para fijar un comandante a mano, crea {ARCHIVO_COMANDANTES}")
        print(f"      junto al script con líneas como:  EDH Urza = Urza, Lord Protector")
    return len(mazos)
...
def exportar_web(rows: list[dict], destino: Path, nombre: str,
                 ...
                 banner: str = "", logo: str = "",
                 rows_oracle: list[dict] | None = None) -> dict:
    ...
    # Una misma impresión repartida en varias carpetas aparecía como filas
    # separadas: "+2 Mace" tres veces en la galería. Las agrupamos por
    # impresión + acabado, sumando copias y conservando todas las carpetas.
    if AGRUPAR_IMPRESIONES:
        agrupadas, orden = {}, []
        for row in rows:
            cid = (row.get("scryfall_id") or "").strip()
            img_tmp = (row.get("image_uri", "") or "").split("?", 1)[0]
            if not cid and img_tmp:
                t = img_tmp.rsplit("/", 1)[-1]
                if t.endswith(".jpg"):
                    cid = t[:-4]
            # La clase entra en la clave: si el mismo Sol Ring está en un mazo
            # y en una carpeta de cambio, son dos entradas distintas. Sin esto,
            # las copias se fusionaban y el destino de todas dependía de cuál
            # apareciera primero en el CSV.
            clave = (cid or f"{row.get('name','')}|{row.get('set_code','')}|"
                            f"{row.get('collector_number','')}",
                     1 if row.get("foil") else 0,
                     clasificar(row))
            if clave in agrupadas:
                base = agrupadas[clave]
                q = row.get("qty", 1)
                base["qty"] = base.get("qty", 1) + (q if isinstance(q, int) else 1)
                for campo in ("binder_name", "condition", "language"):
                    valor = row.get(campo, "")
                    if valor and valor not in base["_multi"][campo]:
                        base["_multi"][campo].append(valor)
            else:
                base = dict(row)
                base["_multi"] = {
                    campo: ([row[campo]] if row.get(campo) else [])
                    for campo in ("binder_name", "condition", "language")
                }
                agrupadas[clave] = base
                orden.append(clave)
        entradas_originales = len(rows)
        rows = [agrupadas[k] for k in orden]
    else:
        entradas_originales = len(rows)
    ...
    for row in rows:
        ...
        multi = row.get("_multi", {})
        binders_fila = multi.get("binder_name") or ([row["binder_name"]] if row.get("binder_name") else [])
        conds_fila   = multi.get("condition")   or ([row["condition"]]   if row.get("condition")   else [])
        langs_fila   = multi.get("language")    or ([row["language"]]    if row.get("language")    else [])
        for binder in binders_fila:
        ...
        try:
            precio = float(row.get("precio_usd") or 0) or None
        except (TypeError, ValueError):
            precio = None
        ...
        filas.append([
            ...
            1 if row.get("foil") else 0,
            qty,
            [i_bind.idx(b) for b in binders_fila],
            [i_cond.idx(c) for c in conds_fila],
            [i_lang.idx(l) for l in langs_fila],
            [i_sub.idx(s) for s in subs],
            i_tipo.idx(sheet_for(row) if sheet_for else "Other"),
            pt,
            precio,
            1 if es_exhibicion(row) else 0,
        ])

        if es_exhibicion(row):
            sld_distintas += 1
            sld_copias += qty
            sld_valor += (precio or 0) * qty
        else:
            total_copias += qty
            valor_total += (precio or 0) * qty
            if row.get("foil"):
                total_foils += qty

    simbolos = descargar_simbolos(destino / "assets" / "simbolos")
    version_assets = datetime.now(timezone.utc).strftime("%Y%m%d%H%M")
    for pagina in ("index.html", "mazos.html", "buscadas.html",
                   "constructor.html", "destape.html", "sld.html"):
        actualizar_version_assets(destino / pagina, version_assets)
    ...
    meta = {
        ...
        "img_base": IMG_BASE,
        "exhibicion": {
            "nombre": " y ".join(BINDERS_EXHIBICION),
            "distintas": sld_distintas,
            "copias": sld_copias,
            "valor_usd": round(sld_valor, 2),
        },
        "totales": {
            "distintas": len(filas) - sld_distintas,
            "entradas": entradas_originales,
            "copias": total_copias,
            "foils": total_foils,
            "sets": len(set_count),
    ...
    # Los textos de reglas cubren TODA la colección, mazos incluidos: es un
    # único diccionario por nombre de carta que sirve a las dos secciones
    for row in (rows_oracle or []):
        n, t = row.get("name", ""), row.get("oracle_text", "") or ""
        if n and t and n not in oracle:
            oracle[n] = t
...
def procesar(input_path: Path):
    ...
    # Las cartas de buscadas.txt entran en el mismo flujo: así reciben datos de
    # Scryfall, caché e imágenes exactamente igual que el resto
    ruta_buscadas = Path(__file__).parent / ARCHIVO_BUSCADAS
    buscadas = parse_buscadas(ruta_buscadas)
    if buscadas:
        proxies = sum(1 for b in buscadas if b["proxy"])
        print(f"  {ARCHIVO_BUSCADAS}: {len(buscadas)} carta(s) "
              f"({proxies} proxies en mazos, {len(buscadas)-proxies} deseos)")
        cards = cards + buscadas
    ...
    con_tmp = cache_abrir(Path(__file__).parent)
    cargar_precios_scg(Path(__file__).parent / ARCHIVO_PRECIOS_SCG, con_tmp)
    if con_tmp:
        con_tmp.close()
    ...
    if REPO_WEB:
        ...
        # Las cartas de mazos armados no son material de intercambio: salen de
        # la portada y van a su propia sección
        grupos = defaultdict(list)
        for r in rows:
            grupos[clasificar(r)].append(r)

        rows_buscadas = grupos["buscada"]
        rows_mazo     = grupos["mazo"]
        rows_col      = grupos["catalogo"] + grupos["vitrina"]
        rows_ocultas  = grupos["oculta"]

        # Los proxies pertenecen a su mazo aunque no estén en tu inventario:
        # sin ellos la lista del mazo no llega a 100 cartas
        rows_mazo = rows_mazo + [r for r in rows_buscadas if es_de_mazo(r)]

        if rows_ocultas:
            copias_ocultas = sum(r["qty"] for r in rows_ocultas if isinstance(r["qty"], int))
            print(f"    {copias_ocultas} carta(s) en binders ocultos "
                  f"({', '.join(BINDERS_OCULTOS[:4])}...) no se publican")
        por_comprar = [r for r in rows_buscadas if es_por_comprar(r)]
        if por_comprar:
            print(f"    {len(por_comprar)} carta(s) de binders de compra "
                  f"pasan a 'Cartas que busco'")
        n_sld = sum(1 for r in rows_col if es_exhibicion(r))
        if n_sld:
            print(f"    {n_sld} carta(s) en vitrina "
                  f"(binder '{BINDERS_EXHIBICION[0]}*') fuera del catálogo")
        if rows_mazo:
            print(f"    {len(rows_mazo)} carta(s) en mazos armados "
                  f"(binder '{PREFIJO_MAZO}*') se excluyen de la portada")

        exportar_web(
            rows_col, REPO_WEB, deck_name, sheet_for=sheet_for,
            ...
            rows_oracle=rows_mazo + rows_buscadas,
        )
        exportar_mazos(rows_mazo, REPO_WEB, sheet_for=sheet_for)
        exportar_buscadas(rows_buscadas, rows_col, REPO_WEB, sheet_for=sheet_for)

        cfg_destape = parse_destape(Path(__file__).parent / ARCHIVO_DESTAPE)
        if cfg_destape["sets"]:
            exportar_destape(grupos["catalogo"] + grupos["vitrina"] + grupos["mazo"],
                             REPO_WEB, cfg_destape, sheet_for=sheet_for)

    # Con --web el HTML autocontenido sobra: son ~10 MB y una descarga del
    # banner que no vas a usar. Se puede forzar añadiendo --html al comando.
    if REPO_WEB and "--html" not in sys.argv:
        print("  (HTML autocontenido omitido; usa --html si lo necesitas)")
        html_generado = False
    else:
        html_generado = True
        print(f"  Generando HTML (modo {modo})...")
    if html_generado and modo == "coleccion":
...
def main():
    ...
    # Control de la caché local
    global USAR_CACHE, REFRESCAR_CACHE
    global PREGUNTAR_COMANDANTE
    if "--sin-preguntar" in sys.argv:
        PREGUNTAR_COMANDANTE = False

    global FUENTE_PRECIO
    if "--precio-scryfall" in sys.argv:
        FUENTE_PRECIO = "scryfall"
    if "--precio-scg" in sys.argv:
        FUENTE_PRECIO = "scg"

    if "--sin-cache" in sys.argv:
        USAR_CACHE = False
        print("  (caché desactivada: se descargará todo de Scryfall)")
    if "--refrescar" in sys.argv or "--precios" in sys.argv:
        REFRESCAR_CACHE = True
        print("  (refrescando: se vuelven a descargar datos y precios)")
